<?php
/**
 * container message envoyés
 * 
 */
?>

<div id='envoyes'class="resume">
    ...
    <?php while ($boiteEnvoi = $messagesEnvoyes->fetch()): ?>
        <span ><ul class="list-group">
                ...
                <li class="list-group-item "><input type="checkbox" name="listeId[]"  id="<?= htmlspecialchars($boiteEnvoi['MESS_ID']) ?>" value="<?= htmlspecialchars($boiteEnvoi['MESS_ID']) ?>"/>
                    ...
                    <span><a href="indexadmin.php?action=newMessage&destinataire=<?= htmlspecialchars($boiteEnvoi['DESTINATAIRE']) ?>
                             " title = "Ecrire à <?= htmlspecialchars($boiteEnvoi['DESTINATAIRE_PSEUDO']) ?>"> A: <?= htmlspecialchars($boiteEnvoi['DESTINATAIRE_PSEUDO']) ?></a>
                    </span>
                    <span ><strong>[Obj: <?= htmlspecialchars($boiteEnvoi['MESS_OBJET']) ?>]</strong></span><br /><span > Mess : <?= $boiteEnvoi['MESS_CONTENT'] ?></li></span>
